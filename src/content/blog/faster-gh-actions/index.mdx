...
_TLDR: The following optimizations made our e2e tests workflow 10x faster. Optimizations included faster tests, more parallelization and more caching._

<hr />

During the development of [BärGPT](https://github.com/technologiestiftung/baergpt), we reached a point where our GitHub e2e test workflow would take over 1 hour to complete. On top of that, the tests were often flaky, which made it even harder to work with. At that point I was fed up and decided to look into it. I was thinking, there surely must be room for improvement? _Oh boy_.
...
The installations of Playwright browsers and Supabase were annoyingly slow. Playwright does [not recommend to cache the browsers](https://playwright.dev/docs/ci#caching-browsers), and caching Supabase does not bring much, as [restoring it from a cache takes as long as downloading it from scratch](https://github.com/supabase/setup-cli/issues/88).
...
After further research, I stumbled upon GitHub Actions' [matrix feature](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/run-job-variations): you can create variations of a job in your workflow.
